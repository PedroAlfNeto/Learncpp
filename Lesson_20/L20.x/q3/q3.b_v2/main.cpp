/*
 Quizz 3: (v2 refactored optimized)
...
int binarySearch(const int* array, int target, int min, int max) {
    ...
    if (value < target) {
        ...
        return binarySearch(array, target, midpoint + 1, max);  // recusion point
    } else if (value > target) {
        // discard top half (including midpoint element already checked)
        return binarySearch(array, target, min, midpoint - 1);  // recusion point
    } else {              // value == target
    ...
    // Never reached
    return -2; // program failure termination condition
}
