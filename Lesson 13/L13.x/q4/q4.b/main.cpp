/*
 ...
 - The deck needs to hold 52 cards, so use a private std::array member to create a fixed array 
    of 52 cards named m_deck. 
 - Second, create a constructor that takes no parameters and initializes/assigns m_deck with 
    one of each card (modify the code from the original createDeck() function). 
 - Third, move printDeck into the Deck class as a public member. 
 - Fourth, move shuffleDeck into the class as a public member.

 The trickiest part of this step is initializing the deck using the modified code from 
    the original createDeck() function.
*/
