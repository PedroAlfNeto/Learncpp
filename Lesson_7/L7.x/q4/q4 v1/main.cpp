/*
 Quizz 4:
	Update your previous solution to handle invalid guesses (e.g. ‘x’), out of
	bounds guesses (e.g. 0 or 101), or valid guesses that have extraneous characters (e.g. 43x).

	Hint: Write a separate function to handle the user inputting their guess
	(along with the associated error handling).
*/

#include "main.hpp"
#include <iostream>
#include <random> // for std::mt19937
#include <limits> // for std::numeric_limits

int main()
{
	std::random_device rd;
	std::seed_seq seq{rd(), rd(), rd(), rd(), rd(), rd(), rd(), rd()};
	std::mt19937 mt{seq};					   // Create a mersenne twister, seeded using the seed sequence
	std::uniform_int_distribution die{1, 100}; // generate random numbers between 1 and 100

	constexpr int guesses{7}; // the user has this many guesses

	do
	{
		int number{die(mt)}; // this is the number the user needs to guess

		std::cout << "Let's play a game. I'm thinking of a number. You have " << guesses << " tries to guess what it is.\n";

		bool won{playGame(guesses, number)};
		if (won)
			std::cout << "Correct! You win!\n";
		else
			std::cout << "Sorry, you lose. The correct number was " << number << "\n";
	} while (playAgain());

	std::cout << "Thank you for playing.\n";

	return 0;
}

// returns true if the user won, false if they lost
bool playGame(int guesses, int number)
{
	// Loop through all of the guesses
	for (int count{1}; count <= guesses; ++count)
	{
		std::cout << "Guess #" << count << ": ";

		int guess{getGuess()};

		if (guess > number)
			std::cout << "Your guess is too high.\n";
		else if (guess < number)
			std::cout << "Your guess is too low.\n";
		else // guess == number
			return true;
	}

	return false; // if the user lost
}

bool playAgain()
{
	// Keep asking the user if they want to play again until they pick y or n.
	while (true)
	{
		char ch{};
		std::cout << "Would you like to play again (y/n)? ";
		std::cin >> ch;

		switch (ch)
		{
		case 'y':
			return true;
		case 'n':
			return false;
		}
	}
}

/* ---- Quizz 4 code ---- */

// Get and validate user guess
int getGuess()
{
	int usrGuess{};
	bool isGuessValid{false};

	do
	{
		std::cin >> usrGuess;
		isGuessValid = validateGuess(usrGuess);

		if (!isGuessValid)
		{
			std::cout << "Try again: ";
		}

		// Clears Cin buffers from any extraction failiure remains and any extraneous data
		clearCinBuffer();

	} while (!isGuessValid);

	return usrGuess;
}

bool validateGuess(int guess)
{

	// Check for invalid input type
	if (std::cin.fail()) // check for cin buffer extraction failiure
	{	
		std::cin.clear(); // put cin in 'normal' operation mode
		std::cerr << "Invalid input type!\n";
		return false;
	}

	// Check bounds
	if ((guess < 1) || (guess > 100))
	{
		std::cerr << "Guess out of bouds (1 to 100)!\n";
		return false;
	}

	// Check for extraneous characters (e.g. 43x)
	
	return true;
}

void clearCinBuffer()
{
	std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // clear buffer for bad input
}
