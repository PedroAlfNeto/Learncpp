/*
 ...
 ---------------------------------------------------------------------------------------------------
 c)  Now we need a way to keep track of which card is next to be dealt
    (in the original program, this is what nextCardIndex was for).
 - First, add a member named m_cardIndex to Deck and initialize it to 0.
 - Create a public member function named dealCard(), which should
    return a const reference to the current card and advance m_cardIndex to the next index.
    shuffle() should also be updated to reset m_cardIndex
        (since if you shuffle the deck, you’ll start dealing from the top of the deck again).
